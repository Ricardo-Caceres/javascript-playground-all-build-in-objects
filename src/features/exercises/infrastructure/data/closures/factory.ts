...
add5(3)`,
    tests: [
      ...
      { description: 'makeAdder(0) returns identity function', assertion: "function makeAdder(n){return x=>x+n} expect(makeAdder(0)(42)).toBe(42)" },
      { description: 'makeAdder works with negative numbers', assertion: "function makeAdder(n){return x=>x+n} expect(makeAdder(-10)(25)).toBe(15)" },
      { description: 'multiple adders are independent', assertion: "function makeAdder(n){return x=>x+n} const add10=makeAdder(10), add5=makeAdder(5); expect(add10(5)).toBe(15); expect(add5(5)).toBe(10)" },
      { description: 'works with decimal numbers', assertion: "function makeAdder(n){return x=>x+n} expect(makeAdder(2.5)(3.5)).toBe(6)" },
    ],
...
mult4(7)`,
    tests: [
      ...
      { description: 'makeMultiplier(1) returns identity', assertion: "function makeMultiplier(n){return x=>x*n} expect(makeMultiplier(1)(42)).toBe(42)" },
      { description: 'makeMultiplier(0) always returns 0', assertion: "function makeMultiplier(n){return x=>x*n} expect(makeMultiplier(0)(999)).toBe(0)" },
      { description: 'works with negative multipliers', assertion: "function makeMultiplier(n){return x=>x*n} expect(makeMultiplier(-3)(4)).toBe(-12)" },
      { description: 'multiple multipliers are independent', assertion: "function makeMultiplier(n){return x=>x*n} const m2=makeMultiplier(2),m3=makeMultiplier(3); expect(m2(5)).toBe(10); expect(m3(5)).toBe(15)" },
    ],
...
greetHello('World')`,
    tests: [
      { description: 'greeter with Hello prefix', assertion: "function makeGreeter(p){return n=>\`\${p} \${n}\`} expect(makeGreeter('Hello')('World')).toBe('Hello World')" },
      { description: 'greeter with Hey prefix', assertion: "function makeGreeter(p){return n=>\`\${p} \${n}\`} expect(makeGreeter('Hey')('Alice')).toBe('Hey Alice')" },
      { description: 'greeter with empty prefix', assertion: "function makeGreeter(p){return n=>\`\${p} \${n}\`} expect(makeGreeter('')('Bob')).toBe(' Bob')" },
      { description: 'multiple greeters with different prefixes', assertion: "function makeGreeter(p){return n=>\`\${p} \${n}\`} const g1=makeGreeter('Hi'),g2=makeGreeter('Goodbye'); expect(g1('John')).toBe('Hi John'); expect(g2('John')).toBe('Goodbye John')" },
      { description: 'works with special characters in name', assertion: "function makeGreeter(p){return n=>\`\${p} \${n}\`} expect(makeGreeter('Welcome')('O\\'Brien')).toBe('Welcome O\\'Brien')" },
    ],
...
isInRange(15)`,
    tests: [
      { description: 'value in range returns true', assertion: "function makeValidator(min,max){return v=>v>=min&&v<=max} const v=makeValidator(10,20); expect(v(15)).toBe(true)" },
      { description: 'value below range returns false', assertion: "function makeValidator(min,max){return v=>v>=min&&v<=max} const v=makeValidator(10,20); expect(v(5)).toBe(false)" },
      { description: 'value above range returns false', assertion: "function makeValidator(min,max){return v=>v>=min&&v<=max} const v=makeValidator(10,20); expect(v(25)).toBe(false)" },
      { description: 'boundary values are included', assertion: "function makeValidator(min,max){return v=>v>=min&&v<=max} const v=makeValidator(10,20); expect(v(10)).toBe(true); expect(v(20)).toBe(true)" },
      { description: 'multiple validators are independent', assertion: "function makeValidator(min,max){return v=>v>=min&&v<=max} const v1=makeValidator(0,10),v2=makeValidator(100,200); expect(v1(5)).toBe(true); expect(v2(150)).toBe(true); expect(v1(150)).toBe(false)" },
    ],
...
square(5)`,
    tests: [
      { description: 'makePower(2) squares numbers', assertion: "function makePower(e){return b=>b**e} const sq=makePower(2); expect(sq(5)).toBe(25)" },
      { description: 'makePower(3) cubes numbers', assertion: "function makePower(e){return b=>b**e} const cb=makePower(3); expect(cb(2)).toBe(8)" },
      { description: 'makePower(0) returns 1', assertion: "function makePower(e){return b=>b**e} const p0=makePower(0); expect(p0(100)).toBe(1)" },
      { description: 'makeCompose applies functions in sequence', assertion: "function makePower(e){return b=>b**e} function makeCompose(fs){return(v)=>fs.reduce((a,f)=>f(a),v)} const sq=makePower(2); const add2=(x)=>x+2; const comp=makeCompose([sq,add2]); expect(comp(3)).toBe(11)" },
      { description: 'makeCompose with multiple functions', assertion: "function makePower(e){return b=>b**e} function makeCompose(fs){return(v)=>fs.reduce((a,f)=>f(a),v)} const add1=(x)=>x+1; const mult2=(x)=>x*2; const comp=makeCompose([add1,mult2,add1]); expect(comp(5)).toBe(13)" },
    ],
