...
emitter.emit('nonexistent')`,
    tests: [
      { description: 'emit does not throw for nonexistent event', assertion: "class E{constructor(){this._l={}}on(e,fn){(this._l[e]??=[]).push(fn)}emit(e,...a){(this._l[e]??=[]).forEach(f=>f(...a))}} const e=new E(); expect(()=>{e.emit('none')}).not.toThrow()" },
      ...
      { description: 'emit works after listening on other event', assertion: "class E{constructor(){this._l={}}on(e,fn){(this._l[e]??=[]).push(fn)}emit(e,...a){(this._l[e]??=[]).forEach(f=>f(...a))}} const e=new E(); e.on('a',()=>{}); expect(()=>{e.emit('b')}).not.toThrow()" },
      { description: 'multiple emits without listeners do not throw', assertion: "class E{constructor(){this._l={}}on(e,fn){(this._l[e]??=[]).push(fn)}emit(e,...a){(this._l[e]??=[]).forEach(f=>f(...a))}} const e=new E(); expect(()=>{e.emit('x');e.emit('y');e.emit('z')}).not.toThrow()" },
...
emitter.emit('test')`,
    tests: [
      ...
      { description: 'off on nonexistent listener does not throw', assertion: "class E{constructor(){this._l={}}on(e,fn){(this._l[e]??=[]).push(fn)}emit(e,...a){(this._l[e]??[]).forEach(f=>f(...a))}off(e,fn){this._l[e]=(this._l[e]??=[]).filter(f=>f!==fn)}} const e=new E(); const h=()=>{}; expect(()=>{e.off('x',h)}).not.toThrow()" },
    ],
