...
const cache = Cache.getInstance()`,
    tests: [
      ...
      { description: 'Database and Cache instances are different objects', assertion: "class D{static i=null;static getInstance(){return D.i??(D.i=new D())}} class C{static i=null;static getInstance(){return C.i??(C.i=new C())}} expect(D.getInstance()).not.toBe(C.getInstance())" },
