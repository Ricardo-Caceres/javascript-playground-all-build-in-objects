...
const desc = new Sorter(descending)`,
    tests: [
      ...
      { description: 'each strategy instance is independent', assertion: "class Sorter{constructor(s){this.s=s}sort(a){return a.slice().sort(this.s)}} const asc=(a,b)=>a-b; const desc=(a,b)=>b-a; const s1=new Sorter(asc); const s2=new Sorter(desc); expect(s1.s).not.toBe(s2.s)" },
    ],
...
const s2 = new Sorter(descending)`,
    tests: [
      ...
      { description: 'each sorter maintains its own strategy', assertion: "class Sorter{constructor(s){this.s=s}sort(a){return a.slice().sort(this.s)}} const asc=(a,b)=>a-b; const desc=(a,b)=>b-a; const s1=new Sorter(asc); const s2=new Sorter(desc); expect(s1.s).not.toBe(s2.s)" },
