...
export const formDataConstructorExercises: Exercise[] = [
  {
    ...
    tests: [
      { description: 'instanceof FormData', assertion: "expect(new FormData() instanceof FormData).toBeTruthy()" },
      { description: 'is truthy', assertion: "expect(new FormData()).toBeTruthy()" },
      { description: 'typeof is object', assertion: "expect(typeof new FormData()).toBe('object')" },
      { description: 'entries starts empty', assertion: "expect(Array.from(new FormData().entries()).length).toBe(0)" },
  ...
  },
  {
    ...
    tests: [
      { description: 'entries length is 0', assertion: "expect(Array.from(new FormData().entries()).length).toBe(0)" },
      { description: 'instanceof FormData', assertion: "expect(new FormData() instanceof FormData).toBeTruthy()" },
      { description: 'typeof is object', assertion: "expect(typeof new FormData()).toBe('object')" },
      { description: 'is truthy', assertion: "expect(new FormData()).toBeTruthy()" },
      { description: 'can append and get', assertion: "const fd=new FormData(); fd.append('k','v'); expect(fd.get('k')).toBe('v')" },
