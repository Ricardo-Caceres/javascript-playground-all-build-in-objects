...
const triple = (x) => multiply(3)(x)`,
    tests: [
      ...
      { description: 'double and triple are reusable', assertion: 'const multiply = (a) => (b) => a * b; const double = (x) => multiply(2)(x); const triple = (x) => multiply(3)(x); expect(double(3)).toBe(6); expect(triple(2)).toBe(6)' },
    ],
