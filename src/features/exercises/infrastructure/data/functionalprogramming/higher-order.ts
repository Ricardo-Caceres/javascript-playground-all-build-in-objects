...
**Challenge:** Implement \`myMap\` that takes an array and a function, applies the function to each element, and returns a new array with the results.`,
  ...
  return arr.reduce((acc, x) => [...acc, fn(x)], [])
    ...
    tests: [
      ...
      { description: 'myMap returns new array', assertion: 'function myMap(arr, fn) { return arr.reduce((acc, x) => [...acc, fn(x)], []) } const orig = [1, 2]; const result = myMap(orig, x => x); expect(result).not.toBe(orig)' },
