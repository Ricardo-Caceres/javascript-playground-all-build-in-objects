...
export const syntaxErrorExercises: Exercise[] = [
  ...
  try {
    ...
    tests: [
      ...
      { description: 'caught error has name SyntaxError', assertion: "function badJson4() { try { JSON.parse('{bad}'); } catch(e: any) { return e.name; } } expect(badJson4()).toBe('SyntaxError')" },
