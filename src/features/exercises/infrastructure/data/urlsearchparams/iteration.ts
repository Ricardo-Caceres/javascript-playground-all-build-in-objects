...
export const urlSearchParamsIterationExercises: Exercise[] = [
  {
    ...
    tests: [
      ...
      { description: 'Array.from keys', assertion:"expect(result).toEqual(['a','b'])" },
      { description: 'keys length matches size', assertion:"expect(result).toHaveLength(2)" },
...
for (const [k, v] of p) {
  ...
  },
  {
    ...
    tests: [
      { description: 'Array.from values', assertion:"expect(result).toEqual(['1','2'])" },
      ...
      { description: 'values length equals size', assertion:"expect(result).toHaveLength(2)" },
    ],
...
[...p.values()]  // → ['1', '2']`,
  ...
  },
  {
    ...
    tests: [
      { description: 'entries returns all pairs as nested arrays', assertion:"expect(result).toEqual([['a','1'],['b','2']])" },
      { description: 'entries returns all pairs', assertion:"expect(result).toHaveLength(2)" },
      { description: 'entries second pair correct', assertion:"expect(result[1]).toEqual(['b','2'])" },
...
p.forEach((val, key) => console.log(key, val))`,
  ...
  },
  {
    ...
    tests: [
      { description: 'forEach collects keys', assertion:"const ks[]=[]; new URLSearchParams('a=1&b=2').forEach((v,k)=>ks.push(k)); expect(ks).toEqual(['a','b'])" },
      { description: 'forEach collects values', assertion:"const vs[]=[]; new URLSearchParams('a=1&b=2').forEach((v)=>vs.push(v)); expect(vs).toEqual(['1','2'])" },
